...
def averageVoxels_parcels(
    ...
    if atlas == "schaefer":
        ...
        # Your original Glasser offset trick to keep hemispheres distinct
        # Left: +1000, Right: +2000, but only where non-zero
        ...
        expected_n = 360  # 180 per hemisphere
...
# Example: Schaefer with your "smallGap" thresholds (will be labeled 'smallGap_Schaefer' on disk)
# for s in subjects:
#     averageVoxels_parcels(subject=s, runNum="run1", analysis_type="smallGap", atlas="schaefer", layer_01=True)

# If/when you want Glasser (keeps 'smallGap' as-is in folder naming):
for s in subjects:
    averageVoxels_parcels(subject=s, runNum="run1", analysis_type="smallGap", atlas="glasser", layer_01=True)

# If you want 8 layers (works for either atlas):
# for s in subjects:
#     averageVoxels_parcels(subject=s, runNum="run1", analysis_type="eightLayers", atlas="schaefer", layer_01=True)